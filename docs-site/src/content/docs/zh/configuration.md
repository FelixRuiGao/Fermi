...
用户全局模板只能**新增**模板，不能覆盖内置的 `explorer` / `executor` / `reviewer` / `main` 模板。要覆盖内置模板，请将其放到**项目本地** `.fermi/agent_templates/`（项目根目录），该位置优先级最高。
...
全局文件位于 `~/.fermi/`；项目文件位于项目根目录。它们的内容会加载进系统提示（会话初始化和重载时重新读取，例如编辑 `AGENTS.md` 或运行 `/reload` 后），因此每轮都可用，代理也可以写入它们。
