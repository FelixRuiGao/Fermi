...
| 级别 | 默认触发 | 代理看到什么 |
|------|----------|--------------|
| Level 1 | 预算的 60% | 提醒调用 `show_context`，考虑总结较早分组 |
| Level 2 | 预算的 80% | 更强提示，要求在 auto-compact 触发前立即总结 |
...
它们在会话初始化和重载时读取（例如编辑 `AGENTS.md` 或运行 `/reload` 后）。代理会读取这些内容作为上下文，也可以写入以保存长期知识。可用 AGENTS.md 存储架构决策、编码约定、已知约束和偏好做法。
