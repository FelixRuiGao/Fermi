...
编辑 MCP 配置后无需重启 Fermi。代理的 `reload` 工具会重新读取配置并按差异应用——新增的服务器会连接、移除的服务器会断开，其 `mcp__*` 工具 schema 也会就地添加或清理。服务器发生变化时，Fermi 会插入一条 `<system-message>`，列出具体哪些 `mcp__server__tool` 条目出现或消失。
...
保存到 `~/.fermi/mcp.json` 后，运行 `/mcp`（或让代理调用 `reload`）连接服务器并验证 SQLite 工具已被发现——无需完整重启。之后代理即可在正常轮次中调用它们。
