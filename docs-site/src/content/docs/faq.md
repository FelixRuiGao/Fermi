...
None. Releases ship as a single self-contained binary — no Bun, Node, or other runtime needed to run Fermi. (Bun 1.3+ is only required if you build from source.)
...
For Kimi, MiniMax, GLM, DeepSeek, Xiaomi, and Qwen, Fermi stores keys in its own internal slots (e.g., `FERMI_KIMI_API_KEY`, `FERMI_QWEN_API_KEY`). External env vars are only imported during `fermi init` or when `/model` prompts for a missing key.
...
- `~/.fermi/AGENTS.md` — global preferences
